...
function test_study_auto()
    ...
    assert_equal("LERNE AUTO Waffenbau", u:get_order(1))
    process_orders()
...
function test_study_auto_expensive()
    ...
    assert_equal("LERNE Magie", u:get_order(1))
    u:clear_orders()
    u:add_order("LERN AUT Taktik")
    assert_equal("LERNE Taktik", u:get_order(1))
    u:clear_orders()
    u:add_order("LERN AUT Waffenbau")
    assert_equal("LERNE AUTO Waffenbau", u:get_order(1))
end
...
function test_faction_stealth()
    ...
    process_orders()
    assert_equal(get_name(u) .. ", getarnt als " .. get_name(f2) .. ", 1 Mensch, aggressiv.", u:show(f))
    assert_equal(get_name(u) .. ", getarnt als " .. get_name(f2) .. ", 1 Mensch.", u:show(f2))
    assert_equal(get_name(u) .. ", " .. get_name(f2) .. ", 1 Mensch.", u:show(f3))
    u:clear_orders()
    u:add_order("TARNE PARTEI NUMMER")
    process_orders()
    assert_equal(get_name(u) .. ", 1 Mensch, aggressiv.", u:show(f))
    assert_equal(get_name(u) .. ", " .. get_name(f) .. ", 1 Mensch.", u:show(f2))
    assert_equal(get_name(u) .. ", " .. get_name(f) .. ", 1 Mensch.", u:show(f3))
end
