...
function test_elvenhorse_requires_riding_5()
    ...
    local goal = region.create(3, 0, "plain")
    local f = create_faction('halfling')
    local u = unit.create(f, r, 1)
...
function test_elvenhorse_without_enough_skill()
    local r = region.create(0, 0, "plain")
    region.create(1, 0, "plain")
...
function test_reserve_and_give()
    region.create(1, 0, "plain")
    local r = region.create(0, 0, "plain")
    local f = create_faction('human')
    local u1 = unit.create(f, r, 1)
    ...
    u1:add_order("RESERVIERE JE 1 Schwert")
    u1:add_order("RESERVIERE 1 Wagen")
    u1:add_order("GIB " .. itoa36(u2.id) .. " ALLES")
    process_orders()
    assert_equal(1, u1:get_item('sword'))
    assert_equal(1, u1:get_item('cart'))
    assert_equal(0, u1:get_item('horse'))
    assert_equal(9, u2:get_item('sword'))
