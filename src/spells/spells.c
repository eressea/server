 ...
 * zufaellig bestimmt, wird aber durch Magiegebiet und Rasse beeinfluﬂt.
 ...
 *  naechsten 6 Runden ein bis 6 Dracheneinheiten bis Groeﬂe Wyrm
 *  entstehen.
 *
...
static int sp_summondragon(castorder * co)
{
  ...
  ADDMSG(&mage->faction->msgs, msg_message("summondragon",
      "unit region command target", mage, mage->region, co->order, co->rt));
 ...
 *  groﬂer Mob, der zur Monsterpartei gehoert und die Region bewacht.
...
static int sp_recruit(castorder * co)
{
   ...
   * mit einfliessen lassen und dafuer den Exponenten etwas groeﬂer.
...
/* ------------------------------------------------------------- */
/* Name:    Wanderprediger - Groﬂe Anwerbung
 * Stufe:   14
 * Gebiet:  Cerddor
 * Wirkung:
 *   Bauern schliessen sich der eigenen Partei an
 ...
 *  erzaehlen, was sie ueber die gefragte Region weiﬂ. Ist in der Region
 *  niemand ihrer Partei, so weiﬂ sie nichts zu berichten.  Auch kann
 ...
 * Gebiet:   Cerddor
 * Wirkung:
 *  Betoert eine Einheit, so das sie ihm den groeﬂten Teil ihres Bargelds
...
static int sp_dragonsong(castorder * co)
{
...
static int sp_songofAttraction(castorder * co)
{
  region *r = co->rt;           /* Zauberregion */
  unit *mage = co->magician.u;
  int cast_level = co->level;
  ...
  ADDMSG(&mage->faction->msgs, msg_message("summon",
      "unit region command region", mage, mage->region, co->order, r));
 ...
 *   Einheiten in der Region so zu beeinflussen, daﬂ sie fuer einige Zeit
