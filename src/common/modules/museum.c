...
/* TODO:
 * - Zweite Ausführung des Benutzes in Zielregion verhindern (Befehl
 *   löschen)
 * - Meldungen
 */
...
create_museum(void)
{
  ...
  assert(terrain_corridor && terrain_hall);
	...
	if(!r) {
		...
		rsetmoney(r, 0);
		rsetpeasants(r, 0);
		set_string(&r->display, "Die Lounge des großen Museums ist ein Platz, in dem sich die Besucher treffen, um die Eindrücke, die sie gewonnen haben, zu verarbeiten. Gemütliche Sitzgruppen laden zum Verweilen ein.");
	}
	...
	if(!r) {
		...
		rsetmoney(r, 0);
		rsetpeasants(r, 0);
		set_string(&r->display, "Die Nördliche Promenade führt direkt in den naturgeschichtlichen Teil des Museums.");
	}
